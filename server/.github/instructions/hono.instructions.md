...
```ts
const route = app.get("/api/posts", (c) => c.json({ posts })).get("/api/posts/:id", (c) => c.json({ post }));
...
```ts
export const app = Hono<BlankEnv, BlankSchema, "/">().get<"foo/:id", "foo/:id", JSONRespondReturn<{ ok: boolean }, 200>, BlankInput, BlankEnv>("foo/:id", (c) => c.json({ ok: true }, 200));
```
...
app.get("/", (c) => {
  return c.html(
    <html>
      <head>{import.meta.env.PROD ? <script type="module" src="/static/client.js"></script> : <script type="module" src="/src/client.ts"></script>}</head>
      <body>
        <h1>Hello</h1>
      </body>
    </html>
...
app.use(async function logger(c, next) {
  ...
  c.req.matchedRoutes.forEach(({ handler, method, path }, i) => {
    ...
    console.log(method, " ", path, " ".repeat(Math.max(10 - path.length, 0)), name, i === c.req.routeIndex ? "<- respond from here" : "");
  });
});
```
